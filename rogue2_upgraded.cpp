...
int* p_v_size = new int; 
int* p_h_size = new int;
...
void init () {
    ...
    level niveau  ;
    niveau.generate() ;
    (*p_v_size) = abs(niveau.x_max-niveau.x_min);
    (*p_h_size) = abs(niveau.y_max-niveau.y_min);
}
...
bool is_external_command(char c){
    return (c == spawn_money);
}
...
void play () {
    ...
    while (c != 'q') {
        ...
        if (is_external_command(c)){
            int valeur_random = rand();
            Gold gold(valeur_random%(*p_v_size),valeur_random%(*p_h_size),10) ;
            gold.affiche();
        }
    }
    delete p_v_size;
